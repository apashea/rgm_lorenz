...
# -----------------------------------------------------------------------------
# Register dataclasses as JAX pytrees (for JIT-friendly passing)
# -----------------------------------------------------------------------------

def _register_pytree_dataclass(cls):
    """Register a @dataclass as a simple pytree: all fields are treated as leaves."""
    fields = list(cls.__dataclass_fields__.keys())

    def flatten(instance):
        children = [getattr(instance, f) for f in fields]
        aux_data = None
...
_register_pytree_dataclass(LorenzLevel)
_register_pytree_dataclass(LorenzHierarchy)
_register_pytree_dataclass(LorenzRGMParams)
...
def build_lorenz_hierarchy(
    ...
    # Spatial metadata from level-0 states_grid
    level0_spatial = spatial_levels[0]
    level0_states_grid = level0_spatial["states_grid"]  # (T0_check, H0, W0)
    T0_check, H0, W0 = level0_states_grid.shape
    assert T0_check == T0, "T0 mismatch between temporal and spatial hierarchies."
    ...
    _ = img_size  # currently unused, kept for API symmetry
    _ = patch_size
    _ = thickness
    _ = O0
...
def build_lorenz_hierarchy_from_params(
    ...
    # Level 0 (patch level) spatial metadata
    level0_states_grid = spatial_levels[0]["states_grid"]
    states_grids.append(level0_states_grid)
    _, H0, W0 = level0_states_grid.shape
