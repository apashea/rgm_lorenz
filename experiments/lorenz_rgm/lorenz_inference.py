...
def vmp_two_level_states(
    ...
    Args:
        ...
        level1: parent-level LorenzLevel (with optional B_states_paths)
        qs0_grid: (T, H0, W0, S0) initial qs at level 0
        states_grid1: (T, H1, W1) layout grid for level 1
        qu_top: (T, U) path posterior; if None, uses uniform over paths
    ...
    # If no paths or no path posterior, fall back to single B1
    ...
    def update_level1(
        ...
        if B_states_paths is not None and qu_current is not None:
            # Build time-varying effective B1_t by mixing over paths
...
# -----------------------------------------------------------------------------
# 5. High-level inference entry point with EFE-based paths (coupled)
# -----------------------------------------------------------------------------
    ...
    Steps:
      ...
      2. If a higher level exists:
           - Run two-level hierarchical state inference using D between
             level 0 and 1 (bottom-up + top-down), with level-1 transitions
             modulated by path posterior qu_top via B_states_paths.
